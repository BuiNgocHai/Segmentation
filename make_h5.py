...
dir_path = '../city/'
img_folder_path = '../city/leftImg8bit/'
gt_folder_path = '../city/gtFine/'
...
def write_data(h5py_file, mode, x_paths, y_paths):
    ...
    for i in range(num_data):
        ...
        x_img = cv2.resize(x_img,(320,256), interpolation=cv2.INTER_CUBIC)
        x_img = cv2.cvtColor(x_img, cv2.COLOR_BGR2RGB)

        y_img = cv2.imread(y_paths[i])
        y_img = cv2.resize(y_img, (320,256), interpolation=cv2.INTER_NEAREST)
        y_img = y_img[:, :, 0]
